...
Result segment(int ny, int nx, const float* data) {
    ...
    std::vector<float>sum((ny+1)*(nx+1));
    // for all rectangles
    ...
    for(int y=0; y<=ny; y++){
        for(int x=0; x<=nx; x++){
            ...
            float rect_sum = sum[y*(nx+1)+x-1]; //3 colors
            for(int j=0; j<y; j++){
                    rect_sum += data[j*nx*3 + (x-1)*3 + 0]; //taking the first color channel
            }
            // double rect_sum[3] = {0};
            // for(int j=0; j<y; j++){
            //     for(int i=0; i<x; i++){
    ...
    float total_sum = sum[(ny)*(nx+1)+(nx)];
    ...
    for(int size=0; size<nx*ny; size++){
        ...
        float reciproc_size_in = 1.0/size_in;
        float reciproc_size_out = 1.0/size_out;
        float reciproc_size_in_out = reciproc_size_out+reciproc_size_in;
        float two_total_sum_size_out = 2*reciproc_size_out*total_sum;
        float hsum_total_sq_sum = reciproc_size_out*(total_sum*total_sum);
        ...
        for(int y0=0; y0<=ny-size_y; y0++){
            for(int x0=0; x0<=nx-size_x; x0++){
                ...
                float sum_in = sum[tmp_y1*new_nx+tmp_x1] - sum[tmp_y1*new_nx+(x0)] - sum[(y0)*new_nx+tmp_x1] + sum[(y0)*new_nx+(x0)];
                // double4_t sum_out = total_sum - sum_in;

                float util = (reciproc_size_in_out*sum_in*sum_in - two_total_sum_size_out*sum_in);
    ...
    float sum_in = sum[(y1+1)*(nx+1)+(x1+1)] - sum[(y1+1)*(nx+1)+(x0)] - sum[(y0)*(nx+1)+(x1+1)] + sum[(y0)*(nx+1)+(x0)];
    float sum_out = total_sum - sum_in;
    ...
    Result result = {
        ...
        best_y1+1,
        best_x1+1,
        {(float)sum_out/size_out, (float)sum_out/size_out, (float)sum_out/size_out},
        {(float)sum_in/size_in, (float)sum_in/size_in, (float)sum_in/size_in}
    };
    sw.record();
    // sw.print();
    // free(sum);
